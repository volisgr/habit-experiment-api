...
class ScoresRequest(BaseModel):
    ...
    experiment_id: str
    date: date
    habit_1: int = Field(ge=0, le=1)
    habit_2: int = Field(ge=0, le=1)
    habit_3: int = Field(ge=0, le=1)
...
def subscribe(req: SubscribeRequest):
    ...
    with psycopg.connect(DATABASE_URL) as conn:
        with conn.cursor() as cur:
            ...
            """, (experiment_id, user_id, start_date, end_date, req.goal))
...
@app.post("/scores")
def post_scores(scores: ScoresRequest):
    DATABASE_URL = os.getenv("DATABASE_URL")
    if not DATABASE_URL: 
        raise HTTPException(500, "DATABASE_URL not set")
    ...
    with psycopg.connect(DATABASE_URL) as conn:
        with conn.cursor() as cur:
            ...
            """, (uuid.UUID(scores.experiment_id), scores.user_id))
            if not cur.fetchone():
            ...
            """, (uuid.UUID(scores.experiment_id), scores.date))
            if cur.fetchone():
            ...
            """, (
                uuid.UUID(scores.experiment_id), 
                scores.user_id,
                scores.date, 
                scores.habit_1, 
                scores.habit_2, 
            ...
            return {
                ...
                "date": scores.date.isoformat()
            }

@app.get("/progress/{user_id}/{experiment_id}", response_model=ProgressResponse)
